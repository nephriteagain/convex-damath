import { boxPiece, movablePieces, piece, players } from "../types";
...
export function checkMovablePieces(
    ...
    if (didCapturedAPiece) {
        const canMultiJump = boardCopyWithJumps.some(
            (box) =>
                box?.piece?.moves &&
        ...
        movablePieces = movablePieces.filter(
            (p) => p.piece.type !== playerToCheck,
        );
        /**
...
function kingMovable(
    ...
    if (move?.playable && move?.piece == undefined) {
        ...
        if (movableArr) {
            const p = boardData.find(
                (box) =>
                    box?.piece?.type === piece?.type &&
                    box?.piece?.value === piece?.value,
            ) as boxPiece;
...
export function movableJump(
    ...
    if (
        moveTopRight?.playable &&
        moveTopRight?.piece != undefined &&
        moveTopRight?.piece?.type != piece?.type &&
        jumpTopRight?.playable &&
        jumpTopRight?.piece == undefined &&
        direction !== "bot left"
    ) {
    ...
    if (
        moveTopLeft?.playable &&
        moveTopLeft?.piece != undefined &&
        moveTopLeft?.piece?.type != piece?.type &&
        jumpTopLeft?.playable &&
        jumpTopLeft?.piece == undefined &&
        direction !== "bot right"
    ) {
    ...
    if (
        moveBotRight?.playable &&
        moveBotRight?.piece != undefined &&
        moveBotRight?.piece?.type != piece?.type &&
        jumpBotRight?.playable &&
        jumpBotRight?.piece == undefined &&
        direction !== "top left"
    ) {
        piece.moves.push(index + 18);
        movableArr &&
    ...
    if (
        moveBotLeft?.playable &&
        moveBotLeft?.piece != undefined &&
        moveBotLeft?.piece?.type != piece?.type &&
        jumpBotLeft?.playable &&
        jumpBotLeft?.piece == undefined &&
        direction !== "top right"
    ) {
        piece.moves.push(index + 14);
        movableArr &&
 ...
 */
function movable(boardData: boxPiece[], index: number, piece: piece) {
    if (piece.type === "z") {
