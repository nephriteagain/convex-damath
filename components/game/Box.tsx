...
export default function Box({
    ...
    const { gameId, pieceToMove, pieceIndex, gameData } = useAppSelector(
        (state) => state.game,
    );
    ...
    return (
        <div
            className={`select-none relative w-full aspect-square flex items-center justify-center ${
                highlighted ? "bg-green-300" : ""
            }`}
            style={
                playable
                    ? {}
                    : {
                          background:
                              "linear-gradient(to top left, #111 0%, rgba(0, 0, 0, 0.80) 75%",
                      }
            }
        ...
        >
            ...
            <div
                className={`absolute ${
                    piece != undefined
                        ? "left-0 top-0  sm:left-[2px] sm:top-[2px]"
                        : ""
                }`}
            >
                <span
                    className={` font-semibold ${
                        piece == undefined ? "text-4xl" : ""
                    }`}
                >
