...
export default forwardRef(function CreateRoom(
    { showSheet }: { showSheet: () => void },
    ...
    if (!joinedLobby) {
        return (
            <Sheet>
                ...
                {lobbyData != undefined && (
                    <Room _id={lobbyData._id} userId={userId} />
                )}
            </Sheet>
        );
    }
    ...
    return (
        <Sheet>
            <SheetTrigger
                className="px-3 py-[0.44rem] text-white bg-green-600  my-1 rounded-md shadow-sm drop-shadow-sm hover:scale-105 active:scale-100 hover:bg-green-800 disabled:opacity-40 transition-all duration-150 w-fit"
                ref={ref}
            >
                Show Room
            </SheetTrigger>
